...
/* Get info on specific named members of structs or unions, and count the number
   of members in a struct, union, or enum.  The offset returned from here
   includes any base-type bitfield adjustments.  */
...
/* ctf_member_next is a _next-style iterator that can additionally traverse into
   ...
   CTF_MEMBER_ERR on end-of-iteration or error.  The bit-width and offset
   include any base-type bitfield adjustments.  */
...
/* Add a member to a struct or union, either at the next available offset (with
   ...
   with a specific encoding (currently broken due to the rmeoval of slices).
   Offsets need not be unique, but must be added in ascending order.
   ctf_add_member_bitfield with a non-negative bit_width will fail unless the
   struct was created with CTF_STRUCT_BITFIELD.  (A negative bit-width means
   "not a bitfield", as with ctf_member_info et al.)  */
