...
typedef enum ctf_v3_kind
  {
    ...
    CTF_V3_K_SLICE    = 14,	/* Variant data is a ctf_slice_v3_t.  */
...
/* Values for CTF_TYPE_KIND() for CTFv4.  Count down from the top of the ID
   space, */
    ...
    CTF_K_BIG    = 30,	/* Prefix type.
			   vlen is high 16 bits of type vlen;
			   size is high 32 bits of type size.  */
    CTF_K_CONFLICTING = 29,	/* Prefix type.  Name is disambiguator for
...
/* Used when the CTF_KIND_{STRUCT,UNION} kind_flag is on, indicating
   bitfields.  The bit-size overrides the size from the underlying encoding;
...
/* The (obsolescent) ctf_archive is a collection of ctf_dict_t's stored
   ...
   describes the mmap()ed archive (and overlaps the first few bytes of it),
   hence the greater care taken with integral types.  All CTF files in an
   archive must have the same data model.  (This is not validated.)

   It is obsolete: the up-to-date format encodes archives via a link field in
   the struct btf_header itself (see above).

   All integers in the ctfa_archive_v1 structure are stored in little-endian byte
   order.

   The dict size uint64_t prepended to dictionaries is one uint64_t too long: it
   contains the length of the size byte too.
...
/* An array of ctfa_ndicts of this structure lies at ctf_archive[sizeof(struct
   ctf_archive_v1)]) and gives the ctfs or names-relative offsets of each name or
   ctf_dict_t.

   The dict is prepended by a uint64_t giving its length (erroneously including
   the length of the uint64_t itself).  The names are just a strtab
   (\0-separated).

   As with the ctf_archive_v1, this structure is obsolescent and is not
   generated by libctf any longer.  */

typedef struct ctf_archive_v1_modent
{
  uint64_t name;
  uint64_t contents;
} ctf_archive_v1_modent_t;

/* The v2 archive consists of a concatenated stream of BTF or CTF dicts, with
   8-byte alignment padding (though the reader can handle any amount of padding
   up to 64 bytes), followed by this magic number, followed by a string table of
   archive member cunames: if the archive has been deduplicated, the first
   member is the null string.  The null bytes ensure that even a corrupted
   strtab in the trailing BTF dict does not run into the cuname strtab.

   If there is a strtab, there are as many member cunames as there are members
   in the archive.  If there is no strtab, the archive members are unnamed, and
   machinery needing cunames (such as the deduplicator) will make one up if the
   caller does not set one via ctf_dict_set_cuname.  */

#define CTFA_MAGIC 0x0073c2f68de1a300
