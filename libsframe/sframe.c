...
sframe_fre_get_end_ip_offset (sframe_func_desc_entry *fdep, unsigned int i,
			      unsigned char *fres)
{
  sframe_frame_row_entry next_fre;
  ...
  if (i < fdep->sfde_func_num_fres - 1)
    {
      err = sframe_decode_fre ((const char*)fres, &next_fre,
			       fre_type, &size);

      /* Sanity check the next FRE.  */
      if (err || !sframe_fre_sanity_check_p (&next_fre))
	return sframe_set_errno (&err, SFRAME_ERR_FRE_INVAL);

      end_ip_offset = next_fre.fre_start_addr - 1;
    }
		 ...
		 sframe_frame_row_entry *frep)
{
  ...
  fres = (unsigned char *) ctx->sfd_fres + fdep->sfde_func_start_fre_off;
  func_start_addr = fdep->sfde_func_start_address;

  for (i = 0; i < fdep->sfde_func_num_fres; i++)
   {
     err = sframe_decode_fre ((const char *)fres, &cur_fre, fre_type, &size);
     if (err)
       return sframe_set_errno (&err, SFRAME_ERR_FRE_INVAL);
