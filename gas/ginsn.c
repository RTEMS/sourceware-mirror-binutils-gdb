...
ginsn_src_print (struct ginsn_src *src)
{
  ...
  switch (src->type)
    ...
    case GINSN_SRC_REG:
      ...
      break;
    case GINSN_SRC_IMM:
      snprintf (src_str, len, "%llu, ",
		(long long unsigned int) ginsn_get_src_imm (src));
      break;
    case GINSN_SRC_INDIRECT:
      snprintf (src_str, len, "[%%r%d+%llu], ", ginsn_get_src_reg (src),
		(long long unsigned int) ginsn_get_src_disp (src));
      break;
    default:
      break;
    }
...
ginsn_dst_print (struct ginsn_dst *dst)
{
  ...
  if (dst->type == GINSN_DST_REG)
    ...
    {
      char *buf = XNEWVEC (char, 32);
      sprintf (buf, "[%%r%d+%llu]", ginsn_get_dst_reg (dst),
		 (long long unsigned int) ginsn_get_dst_disp (dst));
      strcat (dst_str, buf);
    }
