  ...
  if (btf_hdr_len > 0)
    {
	  ...
	  && btf_hdr_len != sizeof (ctf_btf_header_nolayout_t))
	{
	  ctf_err (err_locus (NULL), ENOTSUP,
		      ...
		      const char *format, va_list alist)
{
  ...
  char *msg = NULL;
  char *input_str = NULL;
  const char *cuname = NULL, *errmsg = NULL;
  ...
  /* Two fps may be provided, the 'err fp' on which the error message is wanted,
     ...
     The error passed in is set on the err fp if and only if an error isn't
     already set there (considering ECTF_NEXT_END to not be an error for these
     purposes).  */
  if (!is_warning)
    {
      if (l.type_fp && err == 0 && ctf_errno (l.type_fp) != 0
	  && ctf_errno (l.type_fp) != ECTF_NEXT_END)
	err = ctf_errno (l.type_fp);

      if (l.err_fp && err != 0
	  && (l.err_fp != l.type_fp || ctf_errno (l.err_fp) == 0
	      || ctf_errno (l.err_fp) == ECTF_NEXT_END))
		     ...
		     ctf_error_t *errp)
{
  ...
  if ((asprintf (&prepended_text, "%s%s\n",
		 cew->cew_is_warning ? _("warning: "): _("error: "),
		 cew->cew_text)) < 0)
    ret = cew->cew_text;
  else
    {
...
void
ctf_errwarning_remove (ctf_dict_t *fp, ctf_error_t err)
{
  ctf_list_t *errlist;
  ctf_err_warning_t *cew;
