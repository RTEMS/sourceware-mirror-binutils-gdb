...
ctf_str_write_strtab (ctf_dict_t *fp)
{
  ...
  while ((err = ctf_dynhash_next (fp->ctf_str_atoms, &it, NULL, &v)) == 0)
	...
	continue;
      ...
      /* Strings shared with the parent have all their refs on the parent,
	 too.  */
      if (atom->csa_flags & CTF_STR_ATOM_IN_PARENT && fp->ctf_parent)
	{
	  ...
	  if (parent_atom)
	    {
	      offset = atom->csa_external_offset;
	      ref_atom = parent_atom;
	    }
	}
      /* External strings (CTF only) just use the external offset (an offset in
	 the ELF strtab).  */
      else if (!fp->ctf_serialize.cs_is_btf && atom->csa_external_offset)
	offset = atom->csa_external_offset;

      if (!ctf_assert (fp, offset < prov_offset))
	goto err_strtab;

      ctf_update_refs (&ref_atom->csa_refs, offset);
    }
  if (err != ECTF_NEXT_END)
    {
