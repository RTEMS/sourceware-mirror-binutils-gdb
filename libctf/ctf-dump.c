...
ctf_dump_header (ctf_dict_t *fp, ctf_dump_state_t *state)
{
  ...
  if (!fp->ctf_opened_btf)
      ...
      if (fp->ctf_openflags > 0)
	{
      ...
      if (ctf_dump_header_strfield(fp, state, "Parent name",
				   hp->cth_parent_name) < 0)
	goto err;
    ...
    {
      if (asprintf (&str, "Magic number: 0x%x\n", hp->btf.bth_preamble.btf_magic) < 0)
	goto err;
      ctf_dump_append (fp, state, str);
    }
...
/* Dump all DATASECs with associated vars, and vars not associated with
   datasecs.  */
static int
ctf_dump_vars (ctf_dict_t *fp, ctf_dump_state_t *arg)
{
  ctf_dump_state_t *state = arg;
  char *str = NULL;
  ctf_id_t type;
  ...
  while ((type = ctf_type_kind_next (fp, &next, CTF_K_VAR)) != CTF_ERR)
    {
      if (ctf_variable_datasec (fp, type) != CTF_ERR
	  || ctf_errno (fp) != ECTF_NODATASEC)
	{
