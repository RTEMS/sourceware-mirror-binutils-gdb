 ...
 retry:
  if (!i->i.ctn_type)
	...
	offset = memb[i->ctn_n].ctm_offset;
      ...
      /* Skip nameless padding types.  Keep track of the space consumed (see
	 below).  */
	...
	*membtype = memb[i->ctn_n].ctm_type;
      ...
      if (membname[0] == 0)
	{
      ...
      /* Base type offsets.  These somewhat legacy representations are not part
	 of the struct itself, so do not participate in the offset-gap encoding
	 and do not cause ctn_size to be adjusted.  */

      if (ctf_type_encoding (fp, i->i.ctn_type, &en) < 0
	  && ctf_errno (fp) != ECTF_WRONGKIND)
	{
	  ctf_err (link_type_err_locus (ofp, fp, -1, (ctf_id_t) i->i.ctn_type),
		   ctf_errno (fp), _("during member encoding lookup"));
	  err = ctf_errno (fp);
	  goto end;
	}

      offset += en.cte_offset;

      /* Bit widths only override if the struct itself doesn't specify one:
	 i.e. if this is a non-bitfield struct.  This machinery is largely used
	 for compat opens, in which no bitfield structs exist in any case.  */
      if (bit_width && *bit_width == -1)
	*bit_width = en.cte_bits;
      i->ctn_n++;
...
ctf_type_next (ctf_dict_t *fp, ctf_next_t **it, int *flag, int want_hidden)
{
  ...
  while (i->i.ctn_idx <= fp->ctf_typemax)
	...
	tp = ctf_dtd_lookup (fp, ctf_index_to_type (fp, i->i.ctn_idx))->dtd_buf;
      else
	tp = fp->ctf_txlate[i->i.ctn_idx];

      if ((!want_hidden) && (!LCTF_ISROOT (fp, tp)))
	{
	  i->i.ctn_idx++;
	  continue;
	}
...
ctf_type_kind_next (ctf_dict_t *fp, ctf_next_t **it, ctf_kind_t kind)
{
  ...
  while (i->i.ctn_idx <= fp->ctf_typemax)
	...
	return -1;			/* errno is set for us.  */
	  ...
	  ctf_find_prefix (fp, tp, kind) == NULL)
	{
	  i->i.ctn_idx++;
	  continue;
	}
  ...
  if (!i)
    {
      ...
      if ((datasec = ctf_type_resolve (fp, datasec)) == CTF_ERR)
...
ctf_tag_next (ctf_dict_t *fp, const char *tag, ctf_next_t **it)
{
  ...
  if (!i)
    {
      ...
      if ((tags = ctf_dynhash_lookup (fp->ctf_tags, tag)) == NULL)
  ...
  err = ctf_dynset_next (i->u.ctn_s, &i->ctn_next, &type);
...
ctf_type_size (ctf_dict_t *fp, ctf_id_t type)
{
  ...
  switch (LCTF_KIND (fp, tp))
    ...
    case CTF_K_VAR:
      return ctf_type_size (ofp, suffix->ctt_type);

    default: /* including enums of all kinds, etc */
...
ctf_type_align (ctf_dict_t *fp, ctf_id_t type)
{
  ...
  switch (kind)
    ...
    case CTF_K_VAR:
      return ctf_type_align (ofp, suffix->ctt_type);
	...
	if (kind == CTF_K_STRUCT)
	  {
	    ...
	    n = 1;		/* Only use first member for structs.  */
...
ctf_type_kind_forwarded_tp (ctf_dict_t *fp, const ctf_type_t *tp)
{
  ...
  /* At this stage, forwards to enums, structs, and unions all show up as
     CTF_K_FORWARD.  Disambiguate them.  */
		 ...
		 ctf_dict_t *rfp, ctf_id_t rtype)
{
  ...
  if ((((lkind == CTF_K_ENUM) || (lkind == CTF_K_ENUM64)) && (rkind == CTF_K_INTEGER)) ||
      (((rkind == CTF_K_ENUM) || (rkind == CTF_K_ENUM64)) && (lkind == CTF_K_INTEGER)))
    return 1;
  ...
  switch (lkind)
    ...
    case CTF_K_VAR:
      {
	ctf_id_t lvar = ctf_type_reference (lfp, ltype);
	ctf_id_t rvar = ctf_type_reference (rfp, rtype);

	return (ctf_type_compat (lfp, lvar, rfp, rvar));
      }
    case CTF_K_POINTER:
      {
...
ctf_member_count (ctf_dict_t *fp, ctf_id_t type)
{
  ...
  if (kind == CTF_K_ENUM || kind == CTF_K_ENUM64
      || (prefix = (ctf_type_t *) ctf_find_prefix (fp, tp, CTF_K_BIG)) == NULL)
    return LCTF_VLEN (fp, tp);

  /* Big struct.  Sweep for nameless padding members and remove them.  */
  vlen = ctf_vlen (fp, type, tp, &n);
  ret = n;
  membs = (ctf_member_t *) vlen;
  for (i = 0; i < n; i++)
    {
      if (membs[i].ctm_name == 0 && membs[i].ctm_type == 0)
	ret--;
    }
		 ...
		 ctf_membinfo_t *mip)
{
  ...
  for (i = 0; i < n; i++)
	...
	offset = memb->ctm_offset;
      ...
      /* Skip nameless padding members, if any.  (They do not resolve.)  */
      if (memb->ctm_name == 0 && memb->ctm_type == 0)
	continue;
      ...
      if ((resolved = ctf_type_resolve (fp, memb->ctm_type)) == CTF_ERR)
	{
      ...
      /* Base type offsets.  These somewhat legacy representations are not part
	 of the struct itself, so do not participate in the offset-gap encoding
	 and do not cause total_offset to be adjusted.  */

      if (ctf_type_encoding (fp, memb->ctm_type, &en) < 0
	  && ctf_errno (fp) != ECTF_WRONGKIND)
	return (ctf_set_errno (ofp, ctf_errno (fp)));
...
/* bsearch comparison function for datasec searches.  */

static int
search_datasec_by_offset (const void *key_, const void *arr_, void *arg)
{
  uint32_t *key = (uint32_t *) key_;
  ctf_var_secinfo_t *arr = (ctf_var_secinfo_t *) arr_;
  ctf_var_secinfo_t **closest = (ctf_var_secinfo_t **) arg;
  ...
  if (*key < arr->cvs_offset)
    ...
    {
      if (!*closest
	  || (*closest)->cvs_offset <= arr->cvs_offset)
	*closest = arr;
      return 1;
    }
...
ctf_datasec_var_offset (ctf_dict_t *fp, ctf_id_t datasec, uint32_t offset)
{
  ...
  if (vlen_len == 0)
    return ctf_typed_err (type_err_locus (ofp, datasec), ECTF_NOTYPE, NULL);

  closest = &sec[0];
  if ((el = bsearch_r (&offset, sec, vlen_len, sizeof (ctf_var_secinfo_t),
		       search_datasec_by_offset, &closest)) != NULL
      && (el->cvs_offset == offset))
    return el->cvs_type;

  size = closest->cvs_size;
  if ((size != 0) || (size = ctf_type_size (fp, closest->cvs_type)) >= 0)
    if (closest->cvs_offset < offset && closest->cvs_offset + size > offset)
      return closest->cvs_type;
...
ctf_id_t
ctf_variable_datasec (ctf_dict_t *fp, ctf_id_t var)
{
  ctf_dict_t *ofp = fp;
  void *sec;

  if ((ctf_lookup_by_id (&fp, var, NULL)) == NULL)
    return -1;			/* errno is set for us.  */

  if (ctf_type_kind (fp, var) != CTF_K_VAR)
    return ctf_typed_err (type_err_locus (ofp, var), ECTF_WRONGKIND, NULL);

  if (ctf_dynhash_lookup_kv (fp->ctf_var_datasecs, (void *) (ptrdiff_t) var,
			     NULL, &sec))
    return (ctf_id_t) sec;

  return ctf_typed_err (type_err_locus (ofp, var), ECTF_NOTYPE, NULL);
}
