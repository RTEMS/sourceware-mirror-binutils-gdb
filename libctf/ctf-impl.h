...
typedef struct ctf_dictops
{
  ...
  uint32_t (*ctfo_get_root) (const ctf_type_t *tp);
...
typedef struct ctf_dtdef
{
  ...
  ctf_type_t *dtd_buf;		/* Type nodes plus variable-length data for
				   this type.  */
  size_t dtd_buf_size;		/* Allocated length of dtd_buf in bytes.  */
...
typedef struct ctf_err_warning
{
  ...
  ctf_error_t cew_err;		/* Error code (if any).  */
...
struct ctf_archive_internal
{
  ...
  size_t ctfi_whole_archive_len;    /* Length of the archive, including trailers.  */
  ...
  uint32_t ctfi_refcnt;		    /* Reference count.  */
};
