...
static void ctf_remove_variable (ctf_dict_t *fp, ctf_id_t type, const char *name,
				 int isroot);
static void ctf_remove_datasec (ctf_dict_t *fp, ctf_id_t type, const char *name,
				int isroot);
static void ctf_remove_type_decl_tag (ctf_dict_t *fp, ctf_id_t type,
				      const char *name, int isroot);
...
ctf_add_vlen (ctf_dict_t *fp, ctf_dtdef_t *dtd, size_t vbytes, int additive)
{
  ...
  size_t prefix_size = dtd->dtd_vlen - (unsigned char *) dtd->dtd_buf;
  size_t vlen_size = size - prefix_size;
...
ctf_insert_type_decl_tag (ctf_dict_t *fp, ctf_id_t type, const char *name)
{
  ...
  if ((types = ctf_dynhash_lookup (fp->ctf_tags, name)) == NULL)
	...
	return (ctf_set_errno (fp, ENOMEM));
      ...
      err = ctf_dynhash_cinsert (fp->ctf_tags, name, types);
      if (err != 0)
	{
	  err *= -1;
...
ctf_dtd_insert (ctf_dict_t *fp, ctf_dtdef_t *dtd, int flag, ctf_kind_t kind)
{
  ...
  if (flag == CTF_ADD_ROOT && dtd->dtd_data->ctt_name
      ...
      if (kind != CTF_K_TYPE_TAG && kind != CTF_K_DECL_TAG)
	{
      ...
      else if (ctf_insert_type_decl_tag (fp, dtd->dtd_type, name) < 0)
...
/* DTD removal is more complicated than insertion, because where ctf_dtd_insert
   merely has to insert into DTDs, DTD removal has to undo whatever other things
   were done by the insertion functions.  The removers have to remove from the
   name tables themselves if need be: the DTD is still deleted for them.  */

typedef void (*ctf_remover_f) (ctf_dict_t *fp, ctf_id_t type,
			       const char *name, int isroot);

static const ctf_remover_f ctf_dtd_removers[CTF_K_MAX] =
  {
...
ctf_dtd_delete (ctf_dict_t *fp, ctf_dtdef_t *dtd)
{
  ...
  if (ctf_dtd_removers[name_kind] != NULL)
    ctf_dtd_removers[name_kind] (fp, dtd->dtd_type, name,
				 LCTF_ISROOT (fp, dtd->dtd_buf));
  else
...
ctf_rollback (ctf_dict_t *fp, ctf_snapshot_id_t id)
{
  ...
  /* Work over the list backwards, removing items in reverse order of their
     addition.  */
  ...
  if (!(fp->ctf_flags & LCTF_CHILD) && (fp->ctf_max_children > 0))
  ...
  else
    {
	 ...
	 start over with a newly-reopened parent.  */
	  ...
	  < (fp->ctf_typemax + fp->ctf_header->cth_parent_ntypes))
		 ...
		 size_t vbytes_extra, ctf_type_t **typep)
{
  ...
  if (fp->ctf_typemax + fp->ctf_header->cth_parent_ntypes + 1
      >= pfp->ctf_provtypemax)
    {
  ...
  /* Prohibit addition of a root-visible type that is already present
     ...
     whose names are not identifiers in the C sense at all.  */
		    ...
		    int64_t value)
{
  ...
  root = LCTF_ISROOT (fp, dtd->dtd_buf);
			 ...
			 int bit_width)
{
  ...
  /* Figure out the right offset for naturally-aligned types, if need be,
     and insert additional unnamed members as needed.  Unions always have all
  ...
  if (kind == CTF_K_UNION)
  ...
  else 				/* Struct members. */
    {
      ...
      if (bit_offset == (unsigned long) - 1 && vlen != 0)
	{
	  /* Possibly-nontrivial natural alignment.  */
      ...
      /* Trivial case.  */
      else if (bit_offset == (unsigned long) -1 && vlen == 0)
	bit_offset = 0;
      ...
      while (bit_offset > bound)
	{
	  ...
	  if (ctf_add_member_bitfield (fp, souid, "", 0,
				       off + dtd->dtd_last_offset, -1) < 0)
	    return -1;		/* errno is set for us.  */
			...
			const ctf_encoding_t encoding)
{
  ...
  return ctf_add_member_bitfield (fp, souid, name, type, bit_offset,
				  encoding.cte_bits);
}
...
ctf_add_member_offset (ctf_dict_t *fp, ctf_id_t souid, const char *name,
		       ctf_id_t type, unsigned long bit_offset)
{
  return ctf_add_member_bitfield (fp, souid, name, type, bit_offset, -1);
}
			  ...
			  size_t size, size_t offset)
{
  ...
  if ((((uint32_t) size) != size) || ((offset != (size_t) -1)
				      && (((uint32_t) offset) != offset)))
    return (ctf_set_typed_errno (fp, ERANGE));
  ...
  if (type != 0 && ctf_type_align (fp, type) < 0)
    {
  ...
  if (vlen != 0)
    {
	 ...
	 (perhaps due to the previous type being incomplete), just set the next
	 offset to something aligned mod 8 (we use the cvs_size, but this is
	 optimistic as it is usually zero).  It might be a waste of space but
	 it'll avoid an error and should suffice for a long time to come.  */

      if (offset == (unsigned long) -1)
	{
	  ssize_t bit_offset = roundup (sec[vlen - 1].cvs_offset + 1, 8) * 8;

	  if ((bit_offset = ctf_type_align_natural (fp, sec[vlen - 1].cvs_type,
						    type, sec[vlen - 1].cvs_offset * CHAR_BIT)) < 0)
	    offset = roundup (sec[vlen - 1].cvs_offset
			      + sec[vlen - 1].cvs_size + 1, 8);
	  else
...
static void
ctf_remove_variable (ctf_dict_t *fp, ctf_id_t type, const char *name,
		     int isroot)
{
  ctf_id_t datasec_id;

  if (name != NULL && name[0] != '\0' && isroot)
    ctf_dynhash_remove (ctf_name_table (fp, CTF_K_VAR), name);

  /* Remove from the datasec if need be.  */

  if ((datasec_id = ctf_variable_datasec (fp, type)) != CTF_ERR)
    {
      ctf_dtdef_t *sec_dtd;
      uint32_t kind, kflag;
      size_t vlen;
      ...
      ctf_dynhash_remove (fp->ctf_var_datasecs, (void *) (ptrdiff_t) type);
	...
	return;
      ...
      /* Slow linear search -- nothing else will do here, since datasecs are not
	 sorted by name.  Search backwards because vars are often added in
	 ascending order.  Compact the deleted variable out of the datasec when
	 we find it.  */
      ...
      for (i = vlen; i > 0; i--, j++)
	{
	  if (sec[i - 1].cvs_type == type)
	    {
	      if (j > 0)
		{
		  memmove (&sec[i - 1], &sec[i],
			   sizeof (ctf_var_secinfo_t) * j);
		}
	      sec_dtd->dtd_vlen_size -= sizeof (ctf_var_secinfo_t);
	      sec_dtd->dtd_data->ctt_info = CTF_TYPE_INFO (kind, kflag, vlen - 1);
	      break;
...
static void
ctf_remove_datasec (ctf_dict_t *fp, ctf_id_t type, const char *name,
		    int isroot)
{
  ctf_next_t *i = NULL;
  void *datasec_id;
  ...
  while ((err = ctf_dynhash_next (fp->ctf_var_datasecs, &i, NULL, &datasec_id)) == 0)
    {
      if ((ctf_id_t) datasec_id == type)
	ctf_dynhash_next_remove (&i);
    }
  if (err != ECTF_NEXT_END)
    ctf_err_warn (fp, 0, 0, _("iteration error removing datasec"));
}
		       ...
		       ctf_dict_t *proc_tracking_fp)
{
  ...
  if (tmp != 0)
	...
	return tmp;
	 ...
	 hand it straight back.  */
      ...
      if (ctf_type_kind (tmp_fp, tmp) == kind)
	{
	  if (kind == CTF_K_STRUCT || kind == CTF_K_UNION
	      || kind == CTF_K_ENUM)
	    {
  ...
  switch (kind)
      ...
      else
	{
	  ...
	  if (ctf_type_kind (src_fp, src_type) == CTF_K_ENUM)
