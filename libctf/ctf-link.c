		       ...
		       const char *cuname_prefix)
{
  ...
  char *filename, *keyname, *prefix = NULL;
  ...
  if (cuname_prefix && ((prefix = strdup (cuname_prefix)) == NULL))
    goto oom1;
...
ctf_link_lazy_open (ctf_dict_t *fp, ctf_link_input_t *input)
{
  ...
  /* See ctf_link_add.  */
#if NOBFD
  ctf_err (err_locus (fp), ECTF_NEEDSBFD, _("cannot open %s lazily"), input->clin_filename);
  return -1;
#else
  ...
  if (!input->clin_arc)
	...
	return 0;
      ...
      ctf_err (err_locus (fp), err, _("opening CTF %s failed"), input->clin_filename);
      return -1;
    }
...
ctf_create_per_cu (ctf_dict_t *fp, ctf_dict_t *input, const char *cu_name)
{
  ...
  if ((cu_fp = ctf_dynhash_lookup (fp->ctf_link_outputs, ctf_name)) == NULL
	...
	goto oom;
      ...
      cu_fp->ctf_parent_name = _CTF_SECTION;
...
ctf_link (ctf_dict_t *fp, ctf_link_flags_t flags)
{
  ...
  /* If the strict-enumerator flag is off on the parent, turn it off on all the
     children too.  */
  if (!(fp->ctf_flags & LCTF_STRICT_NO_DUP_ENUMERATORS))
    {
      ...
      while ((err = ctf_dynhash_next (fp->ctf_link_outputs, &it,
				      NULL, &out)) == 0)
	{
	  ctf_dict_t *dict = (ctf_dict_t *) out;
	  dict->ctf_flags &= ~LCTF_STRICT_NO_DUP_ENUMERATORS;
	}
      if (err != ECTF_NEXT_END)
	{
	  ctf_warn (err_locus (fp), err, _("iteration error"));
		  ...
		  const char *name, ctf_link_flags_t flags)
{
  ...
  while ((tmp = ctf_archive_next (dict, &it, &cuname, 0, &err)) != NULL)
	...
	input_parent_cuname = ctf_dict_cuname (tmp);
      ...
      else if (ctf_link_add_cu_mapping (fp, cuname, input_parent_cuname) < 0)
	{
	  ctf_dict_close (tmp);
	  ctf_next_destroy (it);
	  return -1;				/* errno is set for us.  */
  ...
  if (fp->ctf_link_memb_name_changer)
    {
	  ...
	  && ctf_dict_set_cuname (fp, transformed_name) < 0)
	{
	  errloc = "cuname transformation";
	  goto err;
	}
    }
