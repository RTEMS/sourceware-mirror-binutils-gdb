				...
				ctf_sect_t *symtypetaballsect, ctf_error_t *errp)
{
  ...
  if (nsymtypetab > 0
      && ((symtypetab = calloc (nsymtypetab, sizeof (*symtypetab))) == NULL))
    goto oom;

  if (nsymtypetaball > 0
      && ((symtypetaball = calloc (nsymtypetaball, sizeof (*symtypetaball))) == NULL))
    goto oom;
...
ctf_type_sect_is_btf (ctf_dict_t *fp, int force_ctf)
{
  ...
  for (dtd = ctf_list_next (&fp->ctf_dtdefs);
	...
	continue;
      ...
      if (kind == CTF_K_FLOAT)
	{
	  ctf_dprintf ("Type %lx is a float: dict is CTF", dtd->dtd_type);
	  return 0;
	}
    }
