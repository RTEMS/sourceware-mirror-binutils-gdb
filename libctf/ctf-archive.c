...
/* Note on datatypes: the datatype known outside this file as ctf_archive_t
   ...
   The on-disk structure has no actual struct associated with it, because
		  ...
		  size_t threshold, ctf_arc_write_flags_t flags)
{
  ...
  if (ctf_dict_cnt == 0)
    {
      ctf_err (err_locus (NULL), ECTF_ARCREATE,
	       _("CTF archives must have at least one member"));
      return ECTF_ARCREATE;
    }
  ...
  for (i = 0; i < ctf_dict_cnt; i++)
      ...
      while (cuname_len > 0)
	{
	  if ((len = write (fd, p, cuname_len)) < 0)
	    {
	      ctf_err (err_locus (ctf_dicts[0]), errno,
		       _("cannot write name of archive member %zi (%s) to archive"),
		       i, cuname);
	      return errno;
	    }
	  cuname_len -= len;
...
/* Write out a CTF archive containing at least one member dictionary.  The
   entries in CTF_DICTS are named by their cunames, unless
	       ...
	       size_t threshold, ctf_arc_write_flags_t flags)
{
  ...
  if ((fd = open (file, O_RDWR | O_CREAT | O_TRUNC | O_CLOEXEC, 0666)) < 0)
    {
      ctf_err (err_locus (ctf_dicts[0]), errno, _("cannot create %s"), file);
      return errno;
    }
  ...
  if (arc_len < sizeof (struct ctf_archive_v1))
    {
      ...
      return ctf_err (err_locus (NULL), EOVERFLOW, "CTF v1 archive header is %zi bytes, "
		      "but input is only %zi bytes", sizeof (struct ctf_archive_v1),
		      arc_len);
    }
			  ...
			  const ctf_sect_t *strsect, ctf_error_t *errp)
{
  ...
  arci->ctfi_whole_archive_len = len;
  ...
  if (v1)
      ...
      if (len < sizeof (ctf_btf_header_t))
	{
      ...
      /* Perhaps this is a raw v1 dict.  If it is, only one dict is
	 permitted.  */

      if (((*(unsigned short *) buf) == CTF_MAGIC)
      ...
      while ((magic = ctf_arc_find_magic (p, len - (p - buf), &strtab)) != NULL)
	{
      ...
      /* v1 cannot ever have a trailing strtab, no matter what magic numbers
	 we might find.  */
      if (v1)
	strtab = 0;
      ...
      if (num_dicts > i)
	{
      ...
      ctf_dprintf ("v%s archive: %zi members\n", v1 ? "1" : "2",
		   arci->ctfi_nmemb);
      ...
      if (strtab)
	{
	  ...
	  for (name = arci->ctfi_names; j < i; j++)
	    {
	      ssize_t space_left = len - (((unsigned char *) name) - buf);
	      size_t namelen;

	      if (space_left >= 0)
		namelen = strnlen (name, space_left);

	      if (space_left < 0 || namelen == (size_t) space_left)
		{
		  ctf_set_open_errno (errp, EOVERFLOW);
		  ctf_err (err_locus (NULL), EOVERFLOW,
			   _("archive name table overflow"));
		  goto err_set;
		}
    ...
    {
      ctf_set_open_errno (errp, EINVAL);
      ctf_err (err_locus (NULL), EINVAL, _("CTF archive internal error: no buffer or dict specified"));
      goto err_set;
    }
...
ctf_arc_close_free (struct ctf_archive_internal *arci)
{
  ...
  /* When invoked by ctf_dict_arc(), the lifetime of archives is reversed:
     the archive is an explicit child of a single dict, is owned by that
  ...
  switch (arci->ctfi_on_close)
    ...
    case FREE_ARCHIVE_UNMAP_ON_CLOSE:
      arc_mmap_unmap (arci->ctfi_archive, arci->ctfi_whole_archive_len, NULL);
      break;
    default:;
...
/* Return the ctf_dict_t with the given name, or NULL if none, setting 'err' if
   non-NULL.  A name of NULL means to open the first dict in the archive.

   Public entry point.  */
ctf_dict_t *
ctf_dict_open (struct ctf_archive_internal *arci, const char *name,
	       ctf_error_t *errp)
{
  size_t index = 0;

  ctf_dprintf ("ctf_dict_open (%s): opening\n", name ? name : _CTF_SECTION);
		  ...
		  const char **name, int skip_parent, ctf_error_t *errp)
{
  ...
  /* Iteration is made a bit more complex by the need to handle ctf_dict_t's
     ...
     if skip_parent is on, they are skipped and the iterator terminates
     immediately.  */
  ...
  if (arci->ctfi_dict && i->ctn_n == 0)
      ...
      if (!skip_parent)
	{
	  ...
	  /* Paranoia: make sure the dict has a cuname.  */

	  if (ctf_dict_cuname (arci->ctfi_dict) == NULL)
	    ctf_dict_set_cuname (arci->ctfi_dict, _CTF_SECTION);
